...
tabview(lv_tabview_create(parent, NULL)), mainColor(imainColor)
{
  ...
  style_btn_bg->body.border.opa = LV_OPA_100;
  style_btn_bg->body.border.color = LV_COLOR_WHITE;
...
void MainDisplay::splashScreen(const lv_img_t* imgPtr, int time) {
  ...
  void (*func)(void*) = [](void* input){
    ...
    lv_anim_t a;
    a.var = img; a.start = 10; a.end = 255;
    ...
    a.time = time/3.0;
    a.end_cb = NULL; a.act_time = 0; a.playback = 1; a.playback_pause = time/3.0; a.repeat = 0; a.repeat_pause = 0;
    lv_anim_create(&a);
