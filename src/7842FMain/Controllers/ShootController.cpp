...
ShootController::ShootController(IntakeController*& iintake, FlywheelController*& iflywheel,
                                 pros::ADIPotentiometer* ihoodSensor, double ibackAngle,
                                 IterativePosPIDController* ihoodPid) :
  intake(iintake),
  flywheel(iflywheel),
  hoodSensor(ihoodSensor),
  backAngle(ibackAngle),
  hoodPid(ihoodPid),
  task(taskFnc, this) {
  topAngles = {{0, 0},   {0.5, 0},  {1, 0},  {1.5, 0},   {2, 0},  {2.5, 0},   {3, 0},   {3.5, 0},
               {4, 3.2}, {4.5, 8},  {5, 7},  {5.5, 5.3}, {6, 6},  {6.5, 6.5}, {7, 6.5}, {7.5, 5.2},
               {8, 6.1}, {8.5, 14}, {9, 12}, {9.5, 10},  {10, 9}, {10.5, 8},  {11, 13}};

  middleAngles = {{0, 0},   {0.5, 0},    {1, 0},   {1.5, 0},    {2, 0},    {2.5, 3},
                  {3, 9.8}, {3.5, 12.6}, {4, 21},  {4.5, 22.4}, {5, 15.1}, {5.5, 13.3},
                  {6, 13},  {6.5, 14.2}, {7, 19},  {7.5, 14.4}, {8, 15.5}, {8.5, 26},
                  {9, 22},  {9.5, 21},   {10, 20}, {10.5, 18},  {11, 23}};
}
...
void ShootController::completeJob() {
  if (stateQueue.back() != defaultState) { stateQueue.pop_back(); }
}
...
void ShootController::addMacro(shootMacros macro) {
  ...
  {
    case shootMacros::off: clearQueue(); break;

    case shootMacros::shootTop: addJobs({reportDone, enableShoot, angleTop}); break;

    case shootMacros::shootMiddle: addJobs({reportDone, enableShoot, angleMiddle}); break;

    case shootMacros::shootBoth:
      addJobs({reportDone, enableShoot, enableShoot, waitForDoubleShot, angleMiddle, enableShoot,
               angleTop});
      break;

    case shootMacros::shootTopPlatform: addJobs({reportDone, enableShoot, angleTopPlatform}); break;
    ...
    case shootMacros::shootBothPlatform:
      addJobs(
        {reportDone, enableShoot, enableShoot, angleMiddlePlatform, enableShoot, angleTopPlatform});
      break;

    case shootMacros::shootOut: addJobs({reportDone, enableShoot, angleOut}); break;

    case shootMacros::shootTarget: addJobs({reportDone, enableShoot, angleTarget}); break;

    case shootMacros::shoot: addJobs({reportDone, enableShoot}); break;

    case shootMacros::angle: addJobs({reportDone, angling}); break;

    case shootMacros::cycle: addJobs({reportDone, cycle}); break;
  }
}
...
void ShootController::setTarget(double target) { targetAngle = target; }

void ShootController::setDistanceToFlag(QLength distance) { distanceToFlag = distance; }
...
void ShootController::run() {
  ...
  while (true) {
    ...
    switch (getCurrentJob()) {
      ...
      case angleTop: angleTo(getTopFlagAngle()); break;

      case angleMiddle: angleTo(getMiddleFlagAngle()); break;

      case angleTopPlatform: angleTo(20); break;

      case angleMiddlePlatform: angleTo(32); break;

      case angleOut: angleTo(40); break;

      case angleTarget: angleTo(targetAngle); break;

      case waitForDoubleShot:
        flywheel->enable();
        intake->enable();
        if (distanceToFlag > 6_ft) { pros::delay(500); }
        completeJob();
        break;
      ...
      case loopJob: addJob(currentJob); break;

      case loopMacro: addMacro(currentMacro); break;
    }
