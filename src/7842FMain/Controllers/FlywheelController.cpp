...
void FlywheelController::run() {
  ...
  flywheelLogger.writeFields(
    {"Time", "Target/4", "Rpm/4", "Accel(rpm/s)", "Power", "D", "Battery", "Temp"});
  sensor->reset();
  ...
  while (true) {
    ...
    if (motorPower != 0 || currentRpm != 0) {
      flywheelLogger.writeLine({std::to_string(pros::millis() / 1000.0),
                                std::to_string(targetRpm / 4), std::to_string(currentRpm / 4),
                                std::to_string((velMath->getAccel()).convert(rpm / second)),
                                std::to_string(motorPower), std::to_string(pid->getD()),
