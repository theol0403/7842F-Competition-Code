...
void driverControl() {
  ...
  // if(mDigital(UP)) {
  // 	robot.model->arcade(rightY, (ipow(std::abs(leftX), 2) * sgn(leftX)) +
  // robot.vision->allign(), 0); } else {
  ...
  if (mDigital(DOWN)) {
    ...
    armState = ArmController::descore;
  } else if (mDigital(L2)) {
    armState = ArmController::down;
  } else {
    if (armState != ArmController::carry) { armState = ArmController::hold; }
  }
   ...
   */
  double flywheelError = robot.flywheel->targetRpm - robot.flywheel->currentRpm;
  if (flywheelError < 10) {
    robot.printer->print(1, "Flywheel Ready: " + std::to_string((int)(flywheelError)));
    display.driverDisplay->setColor(LV_COLOR_LIME);
  } else {
    robot.printer->print(1, "NOT READY: " + std::to_string((int)(flywheelError)));
