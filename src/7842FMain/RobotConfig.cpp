...
void initializeBase() {
  ...
  robot.model = std::make_shared<SkidSteerModel>(
    ...
    std::make_shared<ADIEncoder>('H', 'G'), std::make_shared<ADIEncoder>('F', 'E'), 200, 12000);

  robot.tracker =
    new lib7842::OdomTracker(robot.model, 6.472_in, 2.75_in, 360, lib7842::OdomTracker::mdTracking);

#ifdef F_ROBOT // Competition
  robot.chassis = new lib7842::OdomController(
    robot.tracker,
    new IterativePosPIDController(
      0.004, 0.0002, 0.00018, 0,
      TimeUtilFactory::withSettledUtilParams(20, 5, 50_ms)), // Distance PID - To mm
    new IterativePosPIDController(
      0.007, 0, 0, 0,
      TimeUtilFactory::withSettledUtilParams(50, 10, 100_ms)), // Angle PID - To Degree
    new IterativePosPIDController(
      0.017, 0.00, 0.0005, 0,
      TimeUtilFactory::withSettledUtilParams(2, 2, 100_ms)), // Turn PID - To Degree
    0.03);
#endif
...
void initializeDevices() {
  ...
  robot.intake = new IntakeController(new okapi::Motor(mIntake), new okapi::Motor(mIndexer),
                                      new pros::ADILineSensor('D'), 1);

  robot.flywheel = new FlywheelController(
    robot.intake, new Motor(mFlywheel), new ADIEncoder('A', 'B', false),
    new VelMath(quadEncoderTPR / 3, std::make_shared<okapi::AverageFilter<4>>(), 10_ms,
                std::make_unique<Timer>()),
    new emaFilter(0.015), new lib7842::velPID(0.073, 0.35, 0.04, 0.1), 0.6);

#ifdef F_ROBOT
  robot.shooter =
    new ShootController(robot.intake, robot.flywheel, new pros::ADIPotentiometer('C'), 15,
                        new IterativePosPIDController(0.016, 0, 0, 0, TimeUtilFactory::create()));
#else
  robot.shooter =
    new ShootController(robot.intake, robot.flywheel, new pros::ADIPotentiometer('C'), 75.3 - 6,
                        new IterativePosPIDController(0.04, 0, 0, 0, TimeUtilFactory::create()));
#endif

  robot.arm =
    new ArmController(new okapi::Motor(mArm),
                      new IterativePosPIDController(0.01, 0, 0, 0, TimeUtilFactory::create()));

  // robot.vision = new VisionController(new pros::Vision(mVision), display.main->newTab("Vision"),
  // new IterativePosPIDController(0.006, 0.00, 0.00008, 0, TimeUtilFactory::create()));
