...
void AutonClose_NearCross(void* input)
{
  ...
  chassis.driveToPoint({3.3_ft, 8.5_ft}, 2, makeSettle(2_in)); // Move to shooting position
  robot.intake->setState(IntakeController::intakeBall);

  chassis.turnToPoint({9_ft, 11_ft}); // turn to flag

  robot.shooter->setDistanceToFlag(distanceToPoint({9_ft, 11_ft}));
