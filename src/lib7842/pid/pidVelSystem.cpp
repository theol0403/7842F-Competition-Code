...
velPID::velPID(double Kp, double Kd, double Kf, double emaAlpha) : m_dFilter(emaAlpha) {
  m_Kp = Kp;
  m_Kd = Kd;
  m_Kf = Kf;
}
...
double velPID::calculate(double wantedRPM, double currentRPM) {
  ...
  m_lastError = m_Error;
  // if(m_derivative < 0) m_derivative /= 4; //So it will not drop too much speed if it speeds up
  // suddenly
