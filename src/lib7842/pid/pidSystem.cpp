...
PID::PID(double Kp, double Kd, double emaAlpha, double settledError, double settledD,
         QTime settledTime) :
  m_Kp(Kp),
  m_Kd(Kd),
  m_dEma(emaAlpha),
...
double PID::calculateErr(double ierror) {
  ...
  if (fabs(finalPower) > 1.0) { finalPower = sgn(finalPower) * 1.0; }
...
double PID::getError() { return m_Error; }
...
bool PID::isSettled() { return m_isSettled; }
