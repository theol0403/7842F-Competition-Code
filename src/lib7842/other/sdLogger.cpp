...
void SDLogger::writeFail(std::string name) { std::cerr << name << ": SD Fail" << std::endl; }
...
std::string SDLogger::findPath(std::string name, modes mode) {
  ...
  switch (mode) {
    ...
    case time: {
      ...
      // //%Y%m%dT%H%M%S
      // path = "/usd/" + name + std::to_string(date.year) + std::to_string(date.month) +
      // std::to_string(date.day) + "T" + std::to_string(time.hour) + std::to_string(time.min) +
      // std::to_string(time.sec) + ".csv";
      break;
    }
