...
namespace lib7842
{
  ...
  settleFunction_t OdomController::makeSettle(QAngle threshold) {
    return [=](OdomController* that){ return that->m_angleErr.abs() < threshold; };
  }

  settleFunction_t OdomController::makeSettle(QLength threshold) {
    return [=](OdomController* that){ return that->m_distanceErr.abs() < threshold; };
  }

  settleFunction_t OdomController::makeSettle(QLength distanceThreshold, QAngle angleThreshold){
