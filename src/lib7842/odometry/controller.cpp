 ...
 * Odom Controller
 */
OdomController::OdomController(OdomTracker* itracker, IterativePosPIDController* idistancePid,
                               IterativePosPIDController* ianglePid,
                               IterativePosPIDController* iturnPid, double islewRate) :
  tracker(itracker),
...
bool OdomController::checkEmergencyAbort() {
  ...
  if (distanceErr != 0_in) { return checkAbort(10, 0.5_s); }
  return false;
}
...
void OdomController::driveVectorTask() {
  while (true) {
    ...
    if (enableVector) { tracker->model->tank(lastVelL, lastVelR); }
    pros::delay(10);
  }
}
...
void OdomController::runActions(AsyncActionList actions) {
  for (AsyncActionRef action : actions) {
    action.get().run(this);
  }
}
...
QAngle OdomController::angleToPoint(QPoint point) {
  QAngle angle = (atan2(point.x.convert(inch) - tracker->state.x.convert(inch),
                        point.y.convert(inch) - tracker->state.y.convert(inch)) *
                  radian) -
                 tracker->state.theta;
...
settleFunc_t OdomController::makeSettle(QAngle threshold) {
  return [=](OdomController* that) {
    return that->angleErr.abs() < threshold || that->checkEmergencyAbort();
  };
}

settleFunc_t OdomController::makeSettle(QLength threshold) {
  return [=](OdomController* that) {
    return that->distanceErr.abs() < threshold || that->checkEmergencyAbort();
  };
}
