...
SideController::SideController(OdomController* icontroller, autonSides iside) :
  controller(icontroller), side(iside) {}

AsyncAction SideController::makeAsyncAction() { return AsyncAction().withSide(side); }

void SideController::setState(QPoint point) {
  controller->tracker->setState(mirrorSide(point, side));
}

void SideController::setX(QLength x) { controller->tracker->setX(mirrorSide(x, side)); }
...
turnFunc_t SideController::makeArc(double leftRatio, double rightRatio) {
  return side == autonSides::red ? OdomController::makeArc(leftRatio, rightRatio)
                                 : OdomController::makeArc(rightRatio, leftRatio);
}
void SideController::leftPivot(OdomController* that, double vel) {
  side == autonSides::red ? OdomController::leftPivot(that, vel)
                          : OdomController::rightPivot(that, vel);
}
void SideController::rightPivot(OdomController* that, double vel) {
  side == autonSides::red ? OdomController::rightPivot(that, vel)
                          : OdomController::leftPivot(that, vel);
}
...
void SideController::turn(angleCalc_t turnCalc, turnFunc_t turnFunc, settleFunc_t settleFunc,
                          AsyncActionList actions) {
  controller->turn(turnCalc, turnFunc, settleFunc, actions);
}
void SideController::turnToAngle(QAngle angle, turnFunc_t turnFunc, settleFunc_t settleFunc,
                                 AsyncActionList actions) {
  controller->turnToAngle(mirrorSide(angle, side), turnFunc, settleFunc, actions);
}
void SideController::turnAngle(QAngle angle, turnFunc_t turnFunc, settleFunc_t settleFunc,
                               AsyncActionList actions) {
  controller->turnAngle(mirrorSide(angle, side), turnFunc, settleFunc, actions);
}
void SideController::turnToPoint(QPoint point, turnFunc_t turnFunc, settleFunc_t settleFunc,
                                 AsyncActionList actions) {
  controller->turnToPoint(mirrorSide(point, side), turnFunc, settleFunc, actions);
}

void SideController::driveDistanceAtAngle(QLength distance, angleCalc_t turnCalc, double turnScale,
...
void SideController::driveDistance(QLength distance, settleFunc_t settleFunc,
                                   AsyncActionList actions) {
...
void SideController::driveForTimeAtAngle(int time, double vel, angleCalc_t turnCalc,
...
void SideController::driveToPoint(QPoint targetPoint, double turnScale, settleFunc_t settleFunc,
                                  AsyncActionList actions) {
  controller->driveToPoint(mirrorSide(targetPoint, side), turnScale, settleFunc, actions);
}
void SideController::driveToPoint2(QPoint targetPoint, double turnScale, settleFunc_t settleFunc,
                                   AsyncActionList actions) {
  controller->driveToPoint2(mirrorSide(targetPoint, side), turnScale, settleFunc, actions);
}

void SideController::drivePath(Path path, double turnScale, settleFunc_t moveOnSettle,
...
void SideController::drivePath2(Path path, double turnScale, settleFunc_t moveOnSettle,
                                settleFunc_t finalSettle, AsyncActionList actions) {
  controller->drivePath2(mirrorSide(path, side), turnScale, moveOnSettle, finalSettle, actions);
}

void SideController::driveToPointSimple(QPoint point, settleFunc_t settleFunc,
                                        AsyncActionList actions) {
  int direction = 1;
