...
OdomTracker::OdomTracker(std::shared_ptr<okapi::SkidSteerModel> imodel, QLength chassisWidth,
                         QLength wheelDiam, double ticksPerRotation, trackerFunc_t trackerFunc) :
  model(imodel),
  ...
  m_leftVelMath(ticksPerRotation, std::make_shared<okapi::PassthroughFilter>(), 10_ms,
                std::make_unique<Timer>()),
  m_rightVelMath(ticksPerRotation, std::make_shared<okapi::PassthroughFilter>(), 10_ms,
                 std::make_unique<Timer>()),
  m_task(taskFnc, this) {
  reset();
};
...
void OdomTracker::debug() {
  ...
  printf("left: %d, right: %d\n", (int)model->getSensorVals()[0], (int)model->getSensorVals()[1]);
}
...
double OdomTracker::getAbsAvgVelocity() {
  return (getAbsLeftVelocity() + getAbsRightVelocity()) / 2;
}
...
void OdomTracker::aTracking(OdomTracker* that) { that->m_aTracking(); }
...
void OdomTracker::m_aTracking() {
  ...
  if (dAngle == 0.0_rad) {
    ...
    localOffY =
      2 * sin(dAngle.convert(radian) / 2) *
      ((dAvgInch.convert(inch) / dAngle.convert(radian)) + (m_chassisWidth.convert(inch) / 2)) *
      inch;
  }
  ...
  QLength polarR =
    sqrt(std::pow(localOffX.convert(inch), 2) + std::pow(localOffY.convert(inch), 2)) * inch;
...
void OdomTracker::mTracking(OdomTracker* that) { that->m_mTracking(); }
...
void OdomTracker::m_mTracking() {
  ...
  QLength radius =
    (dTheta == 0_rad) ? 0_in : (dCenterArc.convert(inch) / dTheta.convert(radian)) * inch;

  dX =
    dTheta == 0_rad ? 0_in : (radius - (radius.convert(inch) * cos(dTheta.convert(radian))) * inch);
  dY = dTheta == 0_rad ? dCenterArc : (radius.convert(inch) * sin(dTheta.convert(radian))) * inch;

  state.x = (dX.convert(inch) * cos(state.theta.convert(radian)) +
             dY.convert(inch) * sin(state.theta.convert(radian)) + state.x.convert(inch)) *
            inch;
  state.y = (dY.convert(inch) * cos(state.theta.convert(radian)) -
             dX.convert(inch) * sin(state.theta.convert(radian)) + state.y.convert(inch)) *
            inch;
...
void OdomTracker::mdTracking(OdomTracker* that) { that->m_mdTracking(); }
...
void OdomTracker::m_mdTracking() {
  ...
  state.x = (dX * cos(state.theta.convert(radian)) + dY * sin(state.theta.convert(radian)) +
             state.x.convert(inch)) *
            inch;
  state.y = (dY * cos(state.theta.convert(radian)) - dX * sin(state.theta.convert(radian)) +
