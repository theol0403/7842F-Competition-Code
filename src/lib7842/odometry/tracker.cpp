...
namespace lib7842
{
  ...
  m_leftVelMath(ticksPerRotation, std::make_shared<okapi::PassthroughFilter>(), 10_ms, std::make_unique<Timer>()),
  m_rightVelMath(ticksPerRotation, std::make_shared<okapi::PassthroughFilter>(), 10_ms, std::make_unique<Timer>()),
  m_task(taskFnc, this)
  {
    reset();
  };
