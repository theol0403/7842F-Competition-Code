...
QPoint::QPoint(dPoint ipoint) :
  x(ipoint.x * inch), y(ipoint.y * inch), theta(ipoint.theta * radian) {}
...
dPoint::dPoint(QPoint ipoint) :
  x(ipoint.x.convert(inch)), y(ipoint.y.convert(inch)), theta(ipoint.theta.convert(radian)) {}
...
void Path::add(Path path) {
  for (const QPoint& point : path.wayPoints) {
...
QPoint closest(QPoint current, QPoint target) {
  return closest(current,
                 dPoint {sin(current.theta.convert(radian)), cos(current.theta.convert(radian))},
                 target);
}
