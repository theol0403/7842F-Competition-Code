...
AsyncAction& AsyncAction::withTrigger(QPoint point, QLength distanceThresh, triggerTypes type) {
  m_triggers.push_back({[=](OdomController* that) {
                          return that->distanceToPoint(mirrorSide(point, m_side)) < distanceThresh;
                        },
                        type});
  return *this;
}
...
AsyncAction& AsyncAction::withTrigger(QPoint point, QLength distanceThresh, QAngle angleThresh,
                                      triggerTypes type) {
  m_triggers.push_back({[=](OdomController* that) {
                          return that->distanceToPoint(mirrorSide(point, m_side)) <
                                   distanceThresh &&
                                 that->angleToPoint(mirrorSide(point, m_side)).abs() < angleThresh;
                        },
                        type});
  return *this;
}
...
AsyncAction& AsyncAction::withTrigger(QPoint point, QAngle angleThresh, triggerTypes type) {
  m_triggers.push_back({[=](OdomController* that) {
                          return that->angleToPoint(mirrorSide(point, m_side)).abs() < angleThresh;
                        },
                        type});
  return *this;
}
...
AsyncAction& AsyncAction::withTrigger(QAngle angle, QAngle angleThresh, triggerTypes type) {
  m_triggers.push_back(
    {[=](OdomController* that) {
       return that->tracker->getTheta() > mirrorSide(angle, m_side) - angleThresh &&
              that->tracker->getTheta() < mirrorSide(angle, m_side) + angleThresh;
     },
     type});
  return *this;
}
