...
#define makeOperators(x) x(+) x(-) x(*) x(/)
...
makeOperators(objOperator) makeOperators(doubleOperator)
...
void visionObj::snapToGrid(objAttr attr, int grid, double max) { snapTo(attr, max / grid); }

void visionObj::snapToGridX(int grid) { snapToGrid(objAttr::x, grid, VISION_FOV_WIDTH); }

void visionObj::snapToGridY(int grid) { snapToGrid(objAttr::y, grid, VISION_FOV_HEIGHT); }
...
visionObj::visionObj(pros::vision_object obj) {
  ...
  if (obj.type == pros::E_VISION_OBJECT_COLOR_CODE) { isCode = true; }
