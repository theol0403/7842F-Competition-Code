...
void ObjContainer::addObj(visionObj obj) { objects.push_back(obj); }
...
visionObj ObjContainer::getAvg() { return getTotal() / objects.size(); }

ObjContainer::removeFunc_t
  ObjContainer::makeRemove(objAttr attr, std::function<bool(double, double)> compare, double val) {
  return [=](const visionObj& obj) { return compare(obj.getAttr(attr), val); };
}

ObjContainer::removeFunc_t ObjContainer::makeRemove(objAttr attr, bool inRange, double min,
                                                    double max) {
...
ObjContainer ObjContainer::copy() { return *this; }
...
ObjContainer& ObjContainer::shrinkTo(int size) {
...
ObjContainer& ObjContainer::remove(int start, int end) {
  ...
  if (start < objects.size()) { objects.erase(objects.begin() + start, objects.begin() + end); }
  return *this;
}

ObjContainer& ObjContainer::removeBy(objAttr attr, std::function<bool(double, double)> compare,
                                     double val) {
...
void ObjContainer::checkErrSig() {
  for (auto&& obj : objects) {
    assert(obj.getAttr(objAttr::sig) != VISION_OBJECT_ERR_SIG);
  }
}

int ObjContainer::getCount() { return objects.size(); }
