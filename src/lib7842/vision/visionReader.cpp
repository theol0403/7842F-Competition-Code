...
namespace lib7842
{
  ...
  VisionReader &VisionReader::getCode(pros::vision_color_code_t code) {
    int count = vision->read_by_code(0, code, maxCount, temp.data());
    if(count > maxCount) count = 0; //If there are no objects pros returns a huge number
    //add each object to the master array
    for (int i = 0; i < count; i++) addObj(temp.at(i));
    checkErrSig();
  ...
  VisionReader &VisionReader::getCode(std::initializer_list<pros::vision_color_code_t> codes) {
    for(const int &code : codes) getCode(code);
    return *this;
  }
